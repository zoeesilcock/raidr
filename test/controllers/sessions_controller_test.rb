...
class SessionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "that it renders the form again with invalid credentials" do
    post sessions_url, params: { user: user_credentials.merge(email: 'wrong') }
    assert_select '.error.row', "Email or password was incorrect."
  end

  test "that it saves the user ID in the session with valid credentials" do
  ...
  test "that it clears the session on delete" do
    post sessions_url, params: { user: user_credentials }
    delete sessions_url
    assert_redirected_to root_url
